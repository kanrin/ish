...
dword_t sys_stime(uaddr_t time);
...
dword_t sys_clock_settime(dword_t clock, uaddr_t tp);
...
int_t sys_setitimer(int_t which, uaddr_t new_val, uaddr_t old_val);
uint_t sys_alarm(uint_t seconds);
int_t sys_timer_create(dword_t clock, uaddr_t sigevent_addr, uaddr_t timer_addr);
int_t sys_timer_settime(dword_t timer, int_t flags, uaddr_t new_value_addr, uaddr_t old_value_addr);
...
dword_t sys_nanosleep(uaddr_t req, uaddr_t rem);
dword_t sys_gettimeofday(uaddr_t tv, uaddr_t tz);
dword_t sys_settimeofday(uaddr_t tv, uaddr_t tz);
